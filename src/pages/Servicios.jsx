          ...
          <div className="space-y-16">
            {serviceGroups.map(({ id, title, eyebrow, intro, image, icon: Icon, services }, index) => (
              <section
                key={id}
                id={id}
                className="scroll-mt-24 grid overflow-hidden rounded-[1.5rem] border border-steel-200 bg-steel-50 lg:grid-cols-[.88fr_1.12fr]"
              >
                <div className={`relative min-h-[28rem] ${index % 2 ? 'lg:order-2' : ''}`}>
                  ...
                  <div className="absolute bottom-6 left-6 right-6">
                    ...
                    <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.18em] text-white/80">{eyebrow}</p>
                  </div>
                </div>
                <div className="p-6 sm:p-10 lg:p-12">
                  <h2 className="font-display text-2xl font-bold leading-tight text-ink-900 sm:text-3xl">{title}</h2>
                  <p className="mt-5 text-sm leading-relaxed text-steel-600">{intro}</p>
                  <div className="mt-8 grid gap-3 sm:grid-cols-2">
                    {services.map((service) => (
                      <div key={service} className="flex items-start gap-3 rounded-2xl border border-steel-200 bg-white p-4 text-sm leading-snug text-ink-800 shadow-sm">
              ...
              <StaggerItem key={title}>
                <article className="group h-full overflow-hidden rounded-[1.5rem] border border-steel-200 bg-white shadow-sm">
                  ...
                  <div className="p-6">
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-steel-600">{copy}</p>
                  </div>
                </article>
              </StaggerItem>
            ))}
